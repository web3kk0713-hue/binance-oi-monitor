...
import { FlowMonitorProvider } from './FlowMonitorContext';
import { DirectionSettingsProvider } from './DirectionSettingsContext';
import { PrivatePositionsProvider } from './PrivatePositionsContext';
import type { FlowEvent } from '../shared/flowTypes';
import './privatePositions.css';
import { MetricHelp } from './MetricHelp';

const VALUATION_HELP: Record<string, string> = {
  '合约 OI': '尚未平仓的合约名义金额，按合约原始数量 × 标记价折算汇总为美元。不是实际保证金；每份未平仓合约都有多空双方。',
  'OI 数量变化': '与所选窗口起点相比，归一化的未平仓币数量增减百分比。+5% 表示数量增加5%，不是多头占比或上涨概率。',
  '数量 5m': '过去5分钟的原始OI数量变化，排除价格对美元金额的影响。增加表示未平仓规模扩大，不能单独判断多空。',
  '指数价变化': '当前指数价相对5分钟前的涨跌幅。指数价不是成交价，也不保证可以按这个价格交易。',
  'OI 金额变化': 'OI美元名义金额的涨跌幅，既受未平仓数量影响，也受标记价与折算价格影响；不能全解释为资金流入流出。',
  '流通市值': '估算流通市值 = 币价 × 流通供应量。币价来自币安指数价，供应量来自已标示的外部源，供应量按小时更新。',
  'FDV': '完全稀释估值 = 币价 × 可核实的最大供应量。假设全部最大供应量按当前价格计价；不是已投入资金。最大供应量未知时不补造。',
  'FDV 变化': '相对所选区间实际起点的涨跌幅=(末值÷初值−1)×100%。最大供应量不变时主要跟随价格，不代表流入资金。下方同时显示美元差额；K=千、M=百万、B=十亿。',
  'OI / FDV': 'OI名义金额 ÷ FDV × 100%。80%表示OI相当于FDV的80%，不是市场杠杆、概率或必然反转线。',
  'OI / 市值': 'OI名义金额 ÷ 流通市值 × 100%。用于比较合约规模和现流通估值，不是多空人数或真实账户杠杆。',
  '覆盖率': '实际取得且符合计算条件的数据，占应监测范围的比例。缺失/过期不算0；高覆盖率也不保证源数据绝对准确。',
  '占比变化': '先分别计算起点和终点OI/FDV，再比较。相对变化=(末占比÷初占比−1)×100%；百分点变化=末占比−初占比。例如50%到60%是+20%或+10个百分点。',
};
function ValuationHelp({ label }: { label: string }) { return VALUATION_HELP[label] ? <MetricHelp label={label}>{VALUATION_HELP[label]}</MetricHelp> : null; }
...
const MyPositions = lazy(() => import('./MyPositions'));
const RiskAlertCenter = lazy(() => import('./RiskAlertCenter'));
...
function MarketFlowRow({ label, market, now }: { label: string; market: MarketObservation; now: number }) {
  ...
  return <div className="market-flow-row">
    ...
    <div className="flow-value"><span>主动买额占比 / 5m<MetricHelp label={`${label}主动买额占比`}>浏览器实际收到的滚动5分钟主动买入成交金额 ÷ 总成交金额 ×100%，不是买额÷卖额。51%表示买方主动成交51%、卖方49%，仅略偏买；不是人数占比或上涨概率。合约买入也可能是空头平仓。连续采满5分钟才显示。</MetricHelp></span><strong>{live ? percent(market.buyShare) : '—'}</strong></div>
    <div className="flow-value"><span>买卖价差<MetricHelp label={`${label}买卖价差`}>最优卖价与最优买价之差 ÷ 两者中间价，以bp显示；1 bp = 0.01%。越大通常即时成交成本越高，但不包括手续费，也不代表大额下单的完整滑点。</MetricHelp></span><strong>{quoteFresh && market.spreadBps !== null ? market.spreadBps > 0 && market.spreadBps < 0.01 ? '<0.01 bp' : `${market.spreadBps.toLocaleString('en-US', { maximumFractionDigits: 2 })} bp` : '—'}</strong></div>
...
export default function App() {
  ...
  const [view, setView] = useState<'flow' | 'valuation' | 'changes' | 'positions' | 'risks'>(() => {
    const params = new URLSearchParams(location.search);
    if (params.get('view') === 'positions') return 'positions';
    if (params.get('view') === 'risks') return 'risks';
    if (params.get('view') === 'changes') return 'changes';
    if (params.get('view') === 'valuation' || (params.has('asset') && params.get('view') !== 'flow')) return 'valuation';
    return params.get('view') === 'flow' ? 'flow' : 'changes';
  });
  ...
  useEffect(() => { const url = new URL(location.href); url.searchParams.set('view', view); window.history.replaceState(null, '', url); }, [view]);

  useEffect(() => { const interval = setInterval(() => setNow(Date.now()), view === 'valuation' ? 1_000 : 5_000); return () => clearInterval(interval); }, [view]);
  ...
  useEffect(() => { void loadPushRegistration().then((value) => setPushConnected(Boolean(value))).catch(() => undefined);
    if ('serviceWorker' in navigator) {
      ...
      const onMessage = (event: MessageEvent) => {
        if (event.data?.type === 'select-positions') setView('positions');
  ...
  }, [view, assets, deferredQuery, filter, favorites, sort, settings.thresholds.warning, settings.thresholds.critical, monitor.shortline, now]);
  ...
  const openPositions = useCallback(() => setView('positions'), []);
  const openFlowEvent = useCallback((event: FlowEvent) => { setView('flow'); window.dispatchEvent(new CustomEvent('oi-open-flow-event', { detail: event })); }, []);
  ...
  return <>
    <header className="topbar"><a className="brand" href="./" aria-label="市场监测首页"><span className="brand-mark"><Icon name="chart" size={21}/></span><span>市场监测</span><span className="brand-tag">Binance</span></a><nav aria-label="主导航"><button className={`nav-item ${['flow', 'changes', 'valuation'].includes(view) ? 'active' : ''}`} aria-current={['flow', 'changes', 'valuation'].includes(view) ? 'page' : undefined} onClick={() => setView('changes')}>市场机会</button><button className={`nav-item ${view === 'positions' ? 'active' : ''}`} aria-current={view === 'positions' ? 'page' : undefined} onClick={openPositions}>我的持仓</button><button className={`nav-item ${view === 'risks' ? 'active' : ''}`} aria-current={view === 'risks' ? 'page' : undefined} onClick={() => setView('risks')}>风险提醒</button></nav><div className="header-actions"><button className="header-icon" onClick={() => setDialog('source')} title="持仓估值数据来源" aria-label="持仓估值数据来源"><Icon name="source" /></button><button className="header-settings" aria-label="监测设置" onClick={openSettings}><Icon name="settings" size={18} /><span>设置</span></button></div></header>
    {['flow', 'changes', 'valuation'].includes(view) ? <nav className="market-subnav" aria-label="市场视图"><button aria-pressed={view === 'changes'} onClick={() => setView('changes')}>变化监控</button><button aria-pressed={view === 'flow'} onClick={() => setView('flow')}>异常监控</button><button aria-pressed={view === 'valuation'} onClick={() => setView('valuation')}>OI 与估值</button><button onClick={() => setDialog('distribution')}>市场分布</button></nav> : null}
    <DirectionSettingsProvider><FlowMonitorProvider settings={settings} snapshot={monitor.snapshot}><PrivatePositionsProvider snapshot={monitor.snapshot} settings={settings}>
      <div className="view-panel" hidden={view !== 'flow'}><Suspense fallback={<main className="workspace"><div className="chart-loading">正在连接异常监控…</div></main>}><FlowDashboard settings={settings} snapshot={monitor.snapshot} active={view === 'flow'} historyVersion={monitor.historyVersion} onOpenSettings={openSettings}/></Suspense></div>
      {view === 'changes' ? <Suspense fallback={<main className="workspace"><div className="chart-loading">加载变化监控…</div></main>}><ChangeDashboard settings={settings} snapshot={monitor.snapshot} historyVersion={monitor.historyVersion} error={monitor.error ?? backendIssue ?? null} storageError={monitor.storageError} collecting={monitor.collecting} retryAt={monitor.retryAt} onRefresh={monitor.refresh} onOpenSettings={openSettings}/></Suspense> : null}
      {view === 'positions' ? <Suspense fallback={<main className="workspace"><div className="chart-loading">加载本机持仓…</div></main>}><MyPositions/></Suspense> : null}
      {view === 'risks' ? <Suspense fallback={<main className="workspace"><div className="chart-loading">加载风险记录…</div></main>}><RiskAlertCenter alerts={alerts} onPosition={openPositions} onAsset={select} onFlow={openFlowEvent} onSettings={openSettings}/></Suspense> : null}
    </PrivatePositionsProvider></FlowMonitorProvider></DirectionSettingsProvider>
    {view === 'valuation' ? <main className="workspace" id="market">
      ...
      <section className="metrics-strip" aria-label="全市场摘要"><div className="metric"><span>监测合约 OI <small>USD</small><ValuationHelp label="合约 OI"/></span><strong>{monitor.snapshot ? money(totalOi) : <span className="skeleton number-skeleton" />}</strong><small><i className="legend-dot oi-dot" />{monitor.snapshot ? `${monitor.snapshot.coverage.oi} 个币种取得 OI` : '等待 Binance 源头数据'}</small></div><div className="metric"><span>监测币种</span><strong>{monitor.snapshot ? monitor.snapshot.universe.assets.toLocaleString() : <span className="skeleton short-skeleton" />}<em>个</em></strong><small>{monitor.snapshot ? `${monitor.snapshot.universe.contracts} 个交易中永续合约` : '自动识别交易中的永续合约'}</small></div><div className="metric"><span>达到提醒阈值<ValuationHelp label="OI / FDV"/></span><strong className={alertCount ? 'warning-text' : ''}>{monitor.snapshot ? alertCount : <span className="skeleton short-skeleton" />}<em>个</em></strong><small><i className="legend-dot warning-dot-color" />OI / FDV ≥ {settings.thresholds.warning}%</small></div><div className="metric coverage-metric"><span>可验证 FDV 覆盖<ValuationHelp label="覆盖率"/></span><strong>{monitor.snapshot ? monitor.snapshot.coverage.eligible : <span className="skeleton short-skeleton" />}<em>/ {monitor.snapshot?.universe.assets ?? '—'}</em></strong><small>{monitor.snapshot ? `市值 ${monitor.snapshot.coverage.marketCap} · FDV ${monitor.snapshot.coverage.fdv} · 缺失 ${Math.max(0, monitor.snapshot.universe.assets - monitor.snapshot.coverage.eligible)}` : '供应量缺失时保留空值'}</small></div></section>
      ...
      <div className={`workbench mobile-${mobileView}`}>
        <section className="panel detail-panel" aria-label="标的详情"><div className="panel-heading detail-heading"><div className="selected-heading"><span className="coin-avatar large">{selected?.symbol.slice(0, 1) ?? '—'}</span><div><h2>{selected?.symbol ?? '选择标的'}<span className="selected-name">{selected?.name ?? ''}</span></h2><p>{selected ? <><strong>{tokenPrice(selectedValues?.priceUsd)}</strong><span className="dot-divider"/>{selected.contracts.length} 个合约</> : '从行情列表选择币种'}</p>{selectedValues?.priceUsd != null ? <RetainedValueNote at={selectedDisplay?.retainedAt}/> : null}</div></div><button className="button text-button source-button" aria-label="来源" onClick={() => setDialog('source')}><Icon name="source" size={16}/><span>来源</span></button></div>
          <div className="selected-metrics"><div><span><i className="legend-dot oi-dot"/>合约 OI<ValuationHelp label="合约 OI"/></span><strong>{money(selectedValues?.oiUsd)}</strong>{selectedValues?.oiUsd != null ? <RetainedValueNote at={selectedDisplay?.retainedAt}/> : null}</div><div><span><i className="legend-dot cap-dot"/>流通市值<ValuationHelp label="流通市值"/></span><strong>{money(selectedValues?.marketCapUsd)}</strong>{selectedValues?.marketCapUsd != null ? <RetainedValueNote at={selectedDisplay?.retainedAt}/> : null}</div><div><span><i className="legend-dot fdv-dot"/>FDV<ValuationHelp label="FDV"/></span><strong>{money(selectedValues?.fdvUsd)}</strong>{selectedValues?.fdvUsd != null ? <RetainedValueNote at={selectedDisplay?.retainedAt}/> : null}</div><div><span>OI / FDV<ValuationHelp label="OI / FDV"/></span><strong className={selectedDisplay?.retainedAt != null ? 'muted' : levelClass(selected?.oiToFdv ?? null, settings.thresholds)}>{percent(selectedValues?.oiToFdv)}</strong>{selectedValues?.oiToFdv != null ? <RetainedValueNote at={selectedDisplay?.retainedAt}/> : null}</div></div>
          <section className="shortline-observation" aria-label="5分钟短线观察"><div className="observation-heading"><h3>5 分钟观察</h3><span className={`observation-state ${ready ? 'is-ready' : ''}`}><i/>{ready ? '连续样本就绪' : selectedShortline?.status === 'stale' || selectedShortline?.status === 'ready' ? '样本已过期' : '等待有效样本'}</span></div><div className="shortline-values"><div><span>OI 数量变化<ValuationHelp label="OI 数量变化"/></span><strong className={changeClass(ready ? selectedShortline?.oiQuantityChange ?? null : null)}>{signed(ready ? selectedShortline?.oiQuantityChange ?? null : null)}</strong></div><div><span>指数价变化<ValuationHelp label="指数价变化"/></span><strong className={changeClass(ready ? selectedShortline?.priceChange ?? null : null)}>{signed(ready ? selectedShortline?.priceChange ?? null : null)}</strong></div><div><span>OI 金额变化<ValuationHelp label="OI 金额变化"/></span><strong>{signed(ready ? selectedShortline?.oiUsdChange ?? null : null)}</strong></div></div><p>{ready ? `${clockTime(selectedShortline?.startAt)}–${clockTime(selectedShortline?.endAt)} · ${selectedShortline?.observations} 个采样点` : selectedShortline?.status === 'ready' ? '最新样本已过期，暂停计算变化' : selectedShortline?.reason ?? '需要连续 5 分钟的原始 OI 数量；不以美元 OI 替代。'}</p></section>
          ...
          <div className="history-change-summary" aria-label="已采集区间变化">
            <div><span className="label">OI 金额变化<ValuationHelp label="OI 金额变化"/></span><strong className="value">{history.loading || history.error ? '—' : signed(trend.oiChange)}</strong><span className="detail">{trend.oiDifference === null || history.loading || history.error ? '等待有效对比点' : signedMoney(trend.oiDifference)}</span></div>
            <div><span className="label">FDV 变化<ValuationHelp label="FDV 变化"/></span><strong className="value">{history.loading || history.error ? '—' : signed(trend.fdvChange)}</strong><span className="detail">{trend.fdvDifference === null || history.loading || history.error ? '等待有效对比点' : signedMoney(trend.fdvDifference)}</span></div>
            <div><span className="label">OI / FDV 变化<ValuationHelp label="占比变化"/></span><strong className="value">{history.loading || history.error ? '—' : signed(trend.ratioDifference, ' pp', 4)}</strong><span className="detail">百分点 · 非涨跌幅</span></div>
          </div>
