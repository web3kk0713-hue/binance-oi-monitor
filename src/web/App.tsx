...
export default function App() {
  ...
  return <>
    ...
    {view === 'changes' ? <Suspense fallback={<main className="workspace"><div className="chart-loading">加载变化监控…</div></main>}><ChangeDashboard settings={settings} snapshot={monitor.snapshot} historyVersion={monitor.historyVersion} error={monitor.error} storageError={monitor.storageError} collecting={monitor.collecting} onRefresh={monitor.refresh} onOpenSettings={() => setDialog('settings')}/></Suspense> : null}
    {view === 'valuation' ? <main className="workspace" id="market">
      ...
      <div className={`workbench mobile-${mobileView}`}>
        <section className="panel detail-panel" aria-label="标的详情"><div className="panel-heading detail-heading"><div className="selected-heading"><span className="coin-avatar large">{selected?.symbol.slice(0, 1) ?? '—'}</span><div><h2>{selected?.symbol ?? '选择标的'}<span className="selected-name">{selected?.name ?? ''}</span></h2><p>{selected ? <><strong>{tokenPrice(selected.priceUsd)}</strong><span className="dot-divider"/>{selected.contracts.length} 个合约</> : '从行情列表选择币种'}</p></div></div><button className="button text-button source-button" aria-label="来源" onClick={() => setDialog('source')}><Icon name="source" size={16}/><span>来源</span></button></div>
      ...
      <section className="panel market-panel" aria-label="市场行情"><div className="market-heading"><div><h2>市场行情<span>{filtered.length}</span></h2><p>同币种合约合并展示</p></div><label className="search-field"><Icon name="search" size={16}/><input aria-label="搜索币种或合约" placeholder="搜索币种" value={query} onChange={(e) => setQuery(e.target.value)}/>{query ? <button onClick={() => setQuery('')} aria-label="清空搜索"><Icon name="close" size={14}/></button> : <kbd>/</kbd>}</label></div>
        ...
        <div className="table-scroll"><table><thead><tr><th className="star-column"><span className="sr-only">自选</span></th><th className="asset-column">币种</th>{columns === 'quality' ? <><th>合约数</th><th>数据状态</th><th>OI 更新</th></> : headers.map((header) => <th key={header.key} aria-sort={sort.key === header.key ? sort.direction === 'desc' ? 'descending' : 'ascending' : 'none'}><button onClick={() => changeSort(header.key)} title={header.title}>{header.label}<span className={sort.key === header.key ? 'sort-active' : ''}>{sort.key === header.key && sort.direction === 'asc' ? '↑' : '↓'}</span></button></th>)}</tr></thead><tbody>
          {pageRows.map((row) => <tr key={row.id} className={`${selected?.id === row.id ? 'selected-row' : ''} ${row.alertEligible && current(row, now) && (row.oiToFdv ?? -1) >= settings.thresholds.critical ? 'critical-row' : ''}`}><td className="star-column"><button className={`star-button ${favorites.has(row.id) ? 'is-favorite' : ''}`} onClick={() => toggleFavorite(row.id)} aria-label={`${favorites.has(row.id) ? '取消自选' : '添加自选'} ${row.symbol}`} aria-pressed={favorites.has(row.id)}><Icon name="star" size={16}/></button></td><td className="asset-column"><button className="asset-button" onClick={() => select(row.id)} aria-pressed={selected?.id === row.id}><span><strong>{row.symbol}</strong><small>{!current(row, now) ? '行情过期' : !row.alertEligible ? row.fdvUsd === null ? 'FDV 缺失' : '待核对' : `${row.contracts.length} 个合约`}</small></span><Icon name="chevron" size={12} className="asset-chevron"/></button></td>{columns === 'quality' ? <><td className="numeric">{row.contracts.length}</td><td><button className={`data-status ${!current(row, now) ? 'stale' : !row.alertEligible ? 'incomplete' : 'verified'}`} onClick={() => { select(row.id); setDialog('source'); }} title={row.issues.join('\n') || '已取得可验证行情与供应量'}><i/>{!current(row, now) ? '行情过期' : !row.alertEligible ? row.fdvUsd === null ? 'FDV 缺失' : '待核对' : '可验证'}</button></td><td className="time-column" title={dateTime(row.oiUpdatedAt)}>{age(row.oiUpdatedAt, now)}</td></> : headers.map((header) => <td className="numeric" key={header.key}>{header.key === 'oiQuantityChange' ? <span className={changeClass(shortlineReady(monitor.shortline[row.id], now) ? monitor.shortline[row.id].oiQuantityChange : null)} title={monitor.shortline[row.id]?.reason ?? '等待连续 5 分钟数量样本'}>{signed(shortlineReady(monitor.shortline[row.id], now) ? monitor.shortline[row.id].oiQuantityChange : null)}</span> : header.key === 'oiToFdv' ? <span className={`ratio-pill ${row.alertEligible && current(row, now) ? levelClass(row.oiToFdv, settings.thresholds) : 'unverified'}`}>{percent(row.oiToFdv)}</span> : header.key === 'oiToMarketCap' ? percent(row.oiToMarketCap) : money(row[header.key])}</td>)}</tr>)}
