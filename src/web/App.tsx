...
const PAGE_SIZE = 25;
const RANGES = [{ label: '1 小时', hours: 1 }, { label: '24 小时', hours: 24 }, { label: '3 天', hours: 72 }, { label: '7 天', hours: 168 }, { label: '30 天', hours: 720 }];
...
export default function App() {
  ...
  const [page, setPage] = useState(1); const [hours, setHours] = useState(168);
  ...
  const historyNow = Math.floor(now / 60_000) * 60_000;
    ...
    <main className="workspace" id="market">
      ...
      <div className="charts-grid">
        ...
        <section className="panel detail-panel"><div className="panel-heading detail-heading"><div className="selected-heading"><span className="coin-avatar large">{selected?.symbol.slice(0, 1) ?? '—'}</span><div><h2>{selected?.symbol ?? '币种详情'}<span className="selected-name">{selected?.name ?? '选择币种查看走势'}</span></h2><p>{selected ? <>{selected.contracts.length} 个合约 <span className="dot-divider"/> {tokenPrice(selected.priceUsd)}</> : '真实数据持续积累中'}</p></div></div><button className="button text-button source-button" aria-label="来源" onClick={() => setDialog('source')}><Icon name="source" size={15}/><span>来源</span></button></div>
          ...
          <div className="history-coverage" role="status">{history.loading ? '读取历史…' : history.error ? '历史读取失败，暂不计算变化' : <>{trend.stale ? '最新历史已过期，暂停计算变化' : trend.coversWindow ? `${RANGES.find(range => range.hours === hours)?.label}区间变化` : `仅显示已采集区间，尚不足 ${RANGES.find(range => range.hours === hours)?.label}`}<span>{trend.baseline ? <>起点 {dateTime(trend.baseline.timestamp)} · </> : null}OI 与 FDV 有效分钟点 {trend.validPoints.toLocaleString()} / {trend.expectedPoints.toLocaleString()}</span></>}</div>
      ...
      <footer className="page-footer"><span><i className="legend-dot oi-dot"/>OI / 价格：Binance 官方接口 <span className="dot-divider"/> 供应量：{settings.mode === 'direct' ? 'CoinGecko' : 'CoinMarketCap / CoinGecko'}</span><span>估值每分钟计算 · 供应量每小时刷新 <button onClick={() => setDialog('source')}>查看计算口径<Icon name="external" size={11}/></button></span></footer>
    </main>
