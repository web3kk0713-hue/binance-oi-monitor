...
export function DirectionPanel({ value, replay = false }: { value: DirectionAssessment; replay?: boolean }) {
  return <section className={`direction-panel ${value.bias}`} aria-label="当前5分钟多空建议">
    <div className="direction-heading"><div><span>短线方向建议 · 固定 5m</span><h3>{value.bias === 'long' ? '↗ ' : value.bias === 'short' ? '↘ ' : ''}{value.label}</h3></div>
    ...
    <div className="direction-guardrails"><p><strong>撤销条件</strong>{value.invalidation}</p>
      {value.risks.length ? <p><strong>风险 / 待确认</strong>{value.risks.join('；')}</p> : null}</div>
    <p className="direction-disclaimer">实验性筛选，未回测盈利能力；候选不等于立即开仓。{replay ? '这是当前建议，不是历史事件发生时的建议。' : '数据不足或冲突时观望。'}</p>
    <details className="direction-method"><summary>查看方向规则与数据窗口</summary>
      <p>偏多：单合约 OI 数量 ≥ +5%、价格 &gt; +0.5%、主动买占比 ≥60% 且净主动成交为正。偏空：OI 数量 ≥ +5%、价格 &lt; −0.5%、主动买占比 ≤40% 且净主动成交为负。上述阈值是初始经验筛选线，不是经过收益验证的交易策略。</p>
