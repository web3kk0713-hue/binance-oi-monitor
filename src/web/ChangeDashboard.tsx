...
export function changeRuleSummary(rule: ChangeRule): string {
  ...
  return `${rule.windowMinutes}m · ${conditionText(oiLabel, rule.oi)} · ${rule.combine === 'all' ? '且' : '或'} · ${conditionText('FDV', rule.fdv)}`;
}
...
function ChangeDashboard({ settings, snapshot, historyVersion, error, storageError, collecting, retryAt = 0, onRefresh, onOpenSettings }: {
  ...
  return <main className="workspace change-monitor">
    ...
    <div className="page-heading"><h1>OI / FDV 变化</h1><div className="refresh-controls"><div className="refresh-meta"><strong>{snapshot ? clockTime(snapshot.asOf) : '等待数据'}</strong><span>{cooldown ? `源接口冷却 ${cooldown}s` : collecting ? '正在更新' : 'OI 目标 30 秒采样'}</span></div><button className="button secondary" onClick={() => { onRefresh(); monitor.refresh(); }} disabled={collecting || cooldown > 0}>刷新</button></div></div>
    ...
    <details className="change-filter-disclosure" open={filtersOpen} onToggle={event => setFiltersOpen(event.currentTarget.open)}>
      <summary ref={filterSummary}><strong>筛选条件</strong><span className="change-filter-rule">{changeRuleSummary(rule)}</span>{dirty ? <span className="change-filter-dirty">有未应用修改</span> : null}<span className="change-filter-action">{filtersOpen ? '收起' : '调整'}</span></summary>
    ...
    <section className="change-results" aria-label="变化监控结果">
      ...
      <div className="change-table-scroll"><table><thead><tr>
        <th>币种</th>
        <th>方向 · 5m<MetricHelp label="当前方向建议">固定 5m 的同合约 OI、价格和主动成交共同筛选；不随本页自定义窗口改变。候选不是立即开仓指令，缺数据、过期或信号冲突时观望。</MetricHelp></th>
        <th>持仓 / 价格联动<MetricHelp label="持仓价格联动">只描述所选窗口起止 OI 数量与价格的组合，例如增仓上涨。每笔合约同时有多空双方，标签不证明主力开仓或必然涨跌。</MetricHelp></th>
        <th><button onClick={() => setSort('oi')} aria-label="按OI变化幅度排序">{oiLabel}变化 {sort === 'oi' ? '↓' : ''}</button><MetricHelp label={`${oiLabel}变化`}>{rule.oiBasis === 'quantity' ? '归一化后的未平仓合约数量变化，排除价格影响。' : '未平仓合约的美元名义金额变化，同时含数量和价格影响。'}（终点 ÷ 起点 − 1）×100%，不是资金净流入。</MetricHelp></th>
        <th>价格变化<MetricHelp label="表格价格变化">与 OI、FDV 使用同一对快照的指数价格涨跌幅。这里只看窗口两端，不证明中间连续上涨或下跌。</MetricHelp></th>
        <th><button onClick={() => setSort('fdv')} aria-label="按FDV变化幅度排序">FDV 变化 {sort === 'fdv' ? '↓' : ''}</button><MetricHelp label="FDV 变化">完全稀释估值 = 价格 × 已核实最大供应量。供给不变时涨跌通常接近币价；供给口径变化也会影响它，不是独立的资金流向证据。</MetricHelp></th>
        <th><button onClick={() => setSort('ratio')} aria-label="按占比相对变化排序">OI / FDV 占比 {sort === 'ratio' ? '↓' : ''}</button><MetricHelp label="占比、相对变化与 pp">占比 = OI 美元名义额 ÷ FDV ×100%。从 10% 到 12% 是相对 +20%、增加 2 pp（百分点）；不是将两个涨跌幅相除，也不是账户杠杆。</MetricHelp></th>
        ...
        <th>{retainedCount ? '最近可用' : '当前'} FDV<MetricHelp label="FDV">完全稀释估值，假设最大供应量全部流通时按当前价格计算的估值。最大供应量或身份无法核实时留空，不代表零。</MetricHelp></th>
