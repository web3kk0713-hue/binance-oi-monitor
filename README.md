...
只读 API：`GET /api/v1/history?assetId=binance:BTC&hours=168` 返回资产历史；`GET /api/v1/contracts/BTCUSDT/history?hours=168` 返回原始 OI/价格字符串和各自源时间、实际接收时间。历史从进程实际采集开始积累，不回填不存在的30秒点。
